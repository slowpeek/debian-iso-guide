...
=debian-live-13.2.0-amd64-gnome.iso= comes with the GNOME desktop,
=debian-live-13.2.0-amd64-kde.iso= has KDE Plasma and so on. An exception is
=debian-live-13.2.0-amd64-standard.iso= which is text only. So if you pick the
...
This is where the =debian-13.2.0-amd64-netinst.iso= netinstaller resides. The
...
Here you can see =debian-13.2.0-amd64-DVD-1.iso=. It is like the netinstaller,
...
for the current =13.2.0= release. You can create the rest with =jigdo=
(explained below).
...
#+begin_src sh
  base=https://cdimage.debian.org/cdimage/release/13.2.0/amd64
  for disk in {1..5}; do
      jigdo-lite "$base"/jigdo-dvd/debian-13.2.0-amd64-DVD-"$disk".jigdo || break
  done
#+end_src
...
With the key acquired and the [[https://cdimage.debian.org/cdimage/release/13.2.0/amd64/iso-cd/debian-13.2.0-amd64-netinst.iso][netinstaller]] downloaded, let's validate it. We
need to fetch two small files from *the same location as the iso*: [[https://cdimage.debian.org/cdimage/release/13.2.0/amd64/iso-cd/SHA256SUMS][SHA256SUMS]]
and [[https://cdimage.debian.org/cdimage/release/13.2.0/amd64/iso-cd/SHA256SUMS.sign][SHA256SUMS.sign]]. Put all three files into the same dir. Next, in that dir,
...
#+begin_example
  gpg: Signature made Sat Nov 15 17:34:58 2025 UTC
