...
desktop environments, one per iso. To say, =debian-live-12.1.0-amd64-gnome.iso=
evidently comes with the GNOME desktop, =debian-live-12.1.0-amd64-kde.iso= has
KDE Plasma and so on. An exception is =debian-live-12.1.0-amd64-standard.iso=
...
This is where the =debian-12.1.0-amd64-netinst.iso= netinstaller resides. The
...
Here you can see =debian-12.1.0-amd64-DVD-1.iso=. It is like the netinstaller,
...
#+begin_src sh
  ...
  for disk in {1..5}; do
      jigdo-lite "$base"/jigdo-bd/debian-12.1.0-amd64-BD-"$disk".jigdo || break
  done
#+end_src
