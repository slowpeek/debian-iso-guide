...
evidently comes with the GNOME desktop, =debian-live-12.5.0-amd64-kde.iso= has
...
This is where the =debian-12.5.0-amd64-netinst.iso= netinstaller resides. The
...
Here you can see =debian-12.5.0-amd64-DVD-1.iso=. It is like the netinstaller,
...
for the current =12.5.0= release. You can create the rest with =jigdo=
(explained below).
...
images one can upgrade an offline machine to =12.5.0= from any state down to
stock =12.0.0=.
