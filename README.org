...
=debian-live-12.8.0-amd64-gnome.iso= comes with the GNOME desktop,
=debian-live-12.8.0-amd64-kde.iso= has KDE Plasma and so on. An exception is
=debian-live-12.8.0-amd64-standard.iso= which is text only. So if you pick the
...
This is where the =debian-12.8.0-amd64-netinst.iso= netinstaller resides. The
...
Here you can see =debian-12.8.0-amd64-DVD-1.iso=. It is like the netinstaller,
...
for the current =12.8.0= release. You can create the rest with =jigdo=
(explained below).
...
releases. For example, using just three =debian-update-12.8.0-amd64-DVD-X.iso=
images one can upgrade an offline machine to =12.8.0= from any state down to
stock =12.0.0=.
...
#+begin_src sh
  base=https://cdimage.debian.org/cdimage/release/12.8.0/amd64
  for disk in {1..5}; do
      jigdo-lite "$base"/jigdo-dvd/debian-12.8.0-amd64-DVD-"$disk".jigdo || break
  done
#+end_src
...
With the key acquired and the [[https://cdimage.debian.org/cdimage/release/12.8.0/amd64/iso-cd/debian-12.8.0-amd64-netinst.iso][netinstaller]] downloaded, let's validate it. We
need to fetch two small files from *the same location as the iso*: [[https://cdimage.debian.org/cdimage/release/12.8.0/amd64/iso-cd/SHA256SUMS][SHA256SUMS]]
and [[https://cdimage.debian.org/cdimage/release/12.8.0/amd64/iso-cd/SHA256SUMS.sign][SHA256SUMS.sign]]. Put all three files into the same dir. Next, in that dir,
...
#+begin_example
  gpg: Signature made Sat Nov  9 16:35:02 2024 UTC
...
#+begin_example
  debian-12.8.0-amd64-netinst.iso: OK
#+end_example
