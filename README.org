...
=debian-live-13.4.0-amd64-gnome.iso= comes with the GNOME desktop,
=debian-live-13.4.0-amd64-kde.iso= has KDE Plasma and so on. An exception is
=debian-live-13.4.0-amd64-standard.iso= which is text only. So if you pick the
...
This is where the =debian-13.4.0-amd64-netinst.iso= netinstaller resides. The
...
Here you can see =debian-13.4.0-amd64-DVD-1.iso=. It is like the netinstaller,
but with all desktops bundled. Using this image you can install Debian with
GNOME, KDE etc on an offline machine.

You can notice this image is marked =DVD-1=. Overall there are 27 =DVD= images
for the current =13.4.0= release. You can create the rest with =jigdo=
(explained below).
...
Up to Debian 12, =jigdo-dvd= additionally provided cumulative update images for
point releases. For example, using just four
=debian-update-12.13.0-amd64-DVD-X.iso= images one could update an offline
machine to =12.13.0= from any state down to stock =12.0.0=. *Since Debian 13, the
update image are no longer provided*.
...
#+begin_src sh
  base=https://cdimage.debian.org/cdimage/release/13.4.0/amd64
  for disk in {1..5}; do
      jigdo-lite "$base"/jigdo-dvd/debian-13.4.0-amd64-DVD-"$disk".jigdo || break
  done
#+end_src
...
Debian 13 images are signed with key =6294BE9B=. You can fetch it with such
command:
...
With the key acquired and the [[https://cdimage.debian.org/cdimage/release/13.4.0/amd64/iso-cd/debian-13.4.0-amd64-netinst.iso][netinstaller]] downloaded, let's validate it. We
need to fetch two small files from *the same location as the iso*: [[https://cdimage.debian.org/cdimage/release/13.4.0/amd64/iso-cd/SHA256SUMS][SHA256SUMS]]
and [[https://cdimage.debian.org/cdimage/release/13.4.0/amd64/iso-cd/SHA256SUMS.sign][SHA256SUMS.sign]]. Put all three files into the same dir. Next, in that dir,
...
#+begin_example
  gpg: Signature made Sat Mar 14 17:41:59 2026 UTC
