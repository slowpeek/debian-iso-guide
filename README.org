...
desktop environments, one per iso. To say, =debian-live-12.7.0-amd64-gnome.iso=
evidently comes with the GNOME desktop, =debian-live-12.7.0-amd64-kde.iso= has
KDE Plasma and so on. An exception is =debian-live-12.7.0-amd64-standard.iso=
...
This is where the =debian-12.7.0-amd64-netinst.iso= netinstaller resides. The
...
Here you can see =debian-12.7.0-amd64-DVD-1.iso=. It is like the netinstaller,
...
for the current =12.7.0= release. You can create the rest with =jigdo=
(explained below).
...
releases. For example, using just three =debian-update-12.7.0-amd64-DVD-X.iso=
images one can upgrade an offline machine to =12.7.0= from any state down to
stock =12.0.0=.
...
#+begin_src sh
  base=https://cdimage.debian.org/cdimage/release/12.7.0/amd64
  for disk in {1..5}; do
      jigdo-lite "$base"/jigdo-dvd/debian-12.7.0-amd64-DVD-"$disk".jigdo || break
  done
#+end_src
...
#+begin_example
  gpg: Signature made Sat Aug 31 22:01:11 2024 UTC
